...
module.exports = function setupLoginRoutes (router) {
    ...
    router.get(
        ...
        try {
            ...
            logger.error(error);
            return res.redirect(`${frontendUri}/login_failed`)
        }
        ...
        logger.info(`Redirecting browser to ${frontendUri} with token ${accessToken}`)
        return res.redirect(`${frontendUri}/login_success?access_token=${accessToken}&id=${userData.id}&battletag=${encodeURIComponent(userData.battletag)}`)
    })
